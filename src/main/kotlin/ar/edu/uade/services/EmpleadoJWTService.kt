...
class EmpleadoJWTService (private val application: Application, private val empleadoService: EmpleadoService) {
    ...
    suspend fun createJwtToken(empleadoRequest: EmpleadoRequest): String? {
        val foundEmpleado: Empleado? = empleadoService.findEmpleadoByLegajo(empleadoRequest.legajo)
        return if (foundEmpleado != null && empleadoRequest.password == foundEmpleado.password)
            JWT.create()
                .withAudience(audience)
                .withIssuer(issuer)
                .withClaim("legajo", empleadoRequest.legajo)
